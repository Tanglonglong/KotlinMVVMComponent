...
class VideoImageView @JvmOverloads constructor(
    ...
    init {
        if (attrs != null) {
            context.withStyledAttributes(attrs, R.styleable.VideoImageView) {
                ratio = getFloat(R.styleable.VideoImageView_vimg_ratio, 0.56f)
                widthOffset = getInt(R.styleable.VideoImageView_vimg_woffset, 0)
            }
        }
    }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        setBackgroundResource(com.example.library_common.R.color.transparent)
        // 父容器传过来的宽度方向上的模式
